...
def createBitmap(vram, palette_ram, oam):
    bitmap = [[0 for j in range(2*WIDTH)] for i in range(2*HEIGHT)]

    for row_g in range(2*HEIGHT):
        row = row_g % 240
        sprite_idxs = []
        if SPRITES_EN:
            sprite_idxs = getSpritesInLine(row-1, oam)
        for col_g in range(2*WIDTH):
            col = col_g % 256

            nametbl = None
            if(row_g < 240 and col_g < 256):
                nametbl = NAMETABLE_0
            elif(row_g < 240 and col_g >= 256):
                nametbl = NAMETABLE_1
            elif(row_g >= 240 and col_g < 256):
                nametbl = NAMETABLE_2
            elif(row_g >= 240 and col_g >= 256):
            ...
            if SPRITES_EN:
                bg_color, bg_color_idx = createBackgroundPixel(row, col, vram, palette_ram, nametbl, PATTERN_TBL_1)
...
def main():
    ...
    for filename in os.listdir(test_folder):
        if(filename.endswith(".txt")):
            full_name = test_folder+filename
            vram, palette_ram, oam = parseMemory(full_name)
            bitmap0 = createBitmap(vram, palette_ram, oam)
            trace_lib.createImage(full_name.replace(".txt", ".png"), bitmap0)
